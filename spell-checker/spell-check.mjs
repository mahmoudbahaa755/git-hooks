...
const ignorePatterns = [/package\.json$/i, /package-lock\.json$/i, /\.json$/i];
...
const getEditedFiles = () => {
	try {
		...
		if (originalCount > 0 && filteredFiles.length === 0) {
			console.log(
				chalk.yellow(`ℹ️ All ${originalCount} files were filtered out by ignore patterns.`)
			);
		} else if (originalCount > filteredFiles.length) {
			console.log(
				chalk.yellow(
					`ℹ️ ${originalCount - filteredFiles.length} files filtered out by ignore patterns.`
				)
			);
		}
...
if (editedFiles.length === 0) {
	console.log(chalk.yellow("ℹ️ No files to check after filtering."));
	process.exit(0);
}
